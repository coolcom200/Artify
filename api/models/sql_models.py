...
class UserPg(sql_alchemy_db.Model):
    ...
    email = sql_alchemy_db.Column(sql_alchemy_db.String(MAX_EMAIL_LENGTH), nullable=False,
                                  unique=True)
    passwordHash = sql_alchemy_db.Column(sql_alchemy_db.String, nullable=False)
...
class ProductPg(sql_alchemy_db.Model):
    ...
    uid = sql_alchemy_db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4())
    ownerUid = sql_alchemy_db.Column(UUID(as_uuid=True), sql_alchemy_db.ForeignKey('user.uid'),
                                     nullable=False)
    productName = sql_alchemy_db.Column(sql_alchemy_db.String(MAX_NAME_LENGTH), nullable=False)
    description = sql_alchemy_db.Column(sql_alchemy_db.Text, nullable=False)
    isVisible = sql_alchemy_db.Column(sql_alchemy_db.Boolean, nullable=False)
...
class FilePg(sql_alchemy_db.Model):
    __tablename__ = 'file'
    fileUid = sql_alchemy_db.Column(UUID(as_uuid=True), nullable=False, primary_key=True,
                                    default=uuid.uuid4())
    productUid = sql_alchemy_db.Column(UUID(as_uuid=True), sql_alchemy_db.ForeignKey('product.uid'),
                                       nullable=False)
    fileName = sql_alchemy_db.Column(sql_alchemy_db.String(MAX_NAME_LENGTH), nullable=False)
    filePath = sql_alchemy_db.Column(sql_alchemy_db.Text, nullable=False)
